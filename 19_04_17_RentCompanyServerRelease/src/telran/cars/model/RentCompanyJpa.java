...
public class RentCompanyJpa extends AbstractRentCompany {
	...
	public CarsReturnCode addModel(Model model) {
		...
		ModelJpa modelJpa = new ModelJpa
				(model.getModelName(), model.getGasTank(),
	...
	public CarsReturnCode addCar(Car car) {
		...
		ModelJpa modelJpa = modelRepository.findById(modelName)
				.orElse(null);
		if(modelJpa == null) {
			return CarsReturnCode.NO_MODEL;
		}
		CarJpa carJpa = new CarJpa
				(regNumber, car.getColor(), car.getState(), modelJpa);
	...
	(String regNumber, long licenseId, LocalDate rentDate, int rentDays) {
		...
		RecordJpa recordJpa = new RecordJpa
				(rentDate, rentDays, carJpa, driverJpa);
	...
	public List<Car> getDriverCars(long licenseId) {
		List<Car> cars = recordRepository
				.findByDriverLicenseId(licenseId)
				.map(r->getCarDto(r.getCar()))
				.distinct()
	...
	public List<Driver> getCarDrivers(String regNumber) {
		return recordRepository.findByCarRegNumber(regNumber)
				.map(r->getDriverDto(r.getDriver()))
				.distinct()
				.collect(Collectors.toList());
	}
	...
	public List<Car> getModelCars(String modelName) {
		...
		return carRepository.findByModelModelName(modelName)
				.filter(c->!c.isFlRemoved()&&!isInUse(c.getRegNumber()))
	...
	private boolean isInUse(String regNumber) {
		...
		return recordRepository
				.findByCarRegNumberAndReturnDateNull(regNumber)!=null;
	}

	@Override
	@Transactional(readOnly=true)
	public List<RentRecord> getRentRecordsAtDates(LocalDate from, LocalDate to) {
		return recordRepository.findByRentDateBetween(from, to)
				.map(this::getRentRecordDto)
				.collect(Collectors.toList());
	}
private RentRecord getRentRecordDto(RecordJpa recordJpa) {
	...
	CarJpa carJpa =
			carRepository.findById(regNumber).orElse(null);
	...
	private List<RentRecord> actualRemove(CarJpa carJpa) {
		List<RecordJpa> recordsForRemove = recordRepository
				.findByCarRegNumber(carJpa.getRegNumber())
				.collect(Collectors.toList());
		recordsForRemove.forEach
		(recordRepository::delete);
		carRepository.delete(carJpa);
		
				
		return recordsForRemove.stream()
				.map(this::getRentRecordDto)
				.collect(Collectors.toList());
	}
	...
	public List<RemovedCarData> removeModel(String modelName) {
		List<CarJpa> modelCars =
				carRepository.findByModelModelName(modelName)
				.collect(Collectors.toList());
	...
	public RemovedCarData returnCar(String regNumber, long licenseId, LocalDate returnDate, int damages,
		...
		updateRecord(recordJpa,returnDate,
				damages,tankPercent,carJpa.getModel().getModelName());
	...
	private void updateRecord(RecordJpa record,
			LocalDate returnDate, int damages,
			int tankPercent, String modelName) {
		record.setDamages(damages);
		record.setReturnDate(returnDate);
		record.setTankPercent(tankPercent);
		int delay=(int) ChronoUnit.DAYS.between(record.getRentDate(),
				returnDate)-record.getRentDays();
	...
	(LocalDate fromDate, LocalDate toDate, int fromAge, int toAge) {
		...
		return carRepository.findMostPopularCarModels
				(fromDate, toDate, birthYearFrom, birthYearTo);
	}
	...
	public List<Driver> getMostActiveDrivers() {
		return driverRepository.findMostActiveDrivers().stream()
